...
create_ldap_key()
{
	echo "create the keys: web_server.key,web_server.crt"
        echo "the keys will save in : /etc/pki/CA/"
	cd /etc/pki/CA/
	openssl genrsa 1024 > web_server.key 2> /dev/null
	openssl  req -new -key web_server.key -out ldap_server.csr -batch &> /dev/null
	openssl  ca -config /etc/pki/tls/openssl.cnf  -batch -passin pass:$ca_key_pass -out web_server.crt -infiles ldap_server.csr &> /dev/null
	echo "create finished , please check."
	exit 0
}
...
--create-web-keys	create keys for web server(you should create ca keys first)
